...
ini_set('display_startuo_errors', 0);
	...
	// 1. Obteniendo datos
		...
		$q_datos = "SELECT DISTINCT
								 ...
								 WHERE MD5(PD.codigo_despacho) = '".$cod_despacho."'";
			...
			if ($res_datos = mysqli_query($enlace, $q_datos)){
				if (mysqli_num_rows($res_datos) > 0) {
					...
					while($row_datos = mysqli_fetch_array($res_datos)){
						...
						$correlativo_despacho = ((strlen($codigo_despacho_comercializacion) > 0) ? $codigo_despacho_comercializacion : $codigo_despacho);
	...
	// 2. Arma la estructura de Detalle (agrupada por proveedor minero)
		...
		$q_datos = "SELECT DISTINCT DU.Id AS ID_DISTRIBUCIONUNIDAD,
							         ...
							         (SELECT COUNT(DISTINCT VD_x.lote_id_proveedorminero, VD_x.despacho_id_modalidadenvio)
							           ...
							           WHERE DU_x.Id = DU.Id
							           	 AND MD5(PD_x.codigo_despacho) = '".$cod_despacho."') AS TOTAL_GUIAS,
					           	 */
...
WHERE DU.id_programacion = '".$id_programacion."'
							AND MD5(PD.codigo_despacho) = '".$cod_despacho."'
